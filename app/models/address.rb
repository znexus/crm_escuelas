...
class Address < ActiveRecord::Base
  ...
  def multi_line
    lines = []
    lines <<  if street_1? then street_1 else "Sin dirección" end
    lines << if street_2? then street_2 else "" end
    
    line = ''
    line << if city? then city else "" end
    if region_name
    ...
    lines << if region then region.name else "" end
    
    lines << if country then country.name else "" end
    
    lines << if telefonos then ' T: '+telefonos else "" end
